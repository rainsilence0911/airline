(function() {
	...
	Shader = function(gl, vertexSource, fragmentSource) {
	...
	Shader.create = function(canvas, vertexSource, fragmentSource, options, opt_onError) {
	...
	Shader.prototype = {
		 ...
		 */
		blend: function(sfactor, dfactor) {
			var gl = this.gl;
		...
		animate: function() {
			...
			targetThis.isStop = false;
			
			var lastTime = 0;
			
			function loopFn() {
				
				if (targetThis.isStop) {
					return;
				}
				
				var timeNow = new Date().getTime();
				
				if (targetThis.ondraw) {
					targetThis.ondraw();
				}
				
				if (targetThis.onupdate) {
				
					if (lastTime != 0) {
						targetThis.onupdate((timeNow - lastTime) / 1000);
					}
					lastTime = timeNow;
				}
		...
		createFrameBufferTexture: function(width, height) {
			var gl = this.gl;
...
//							gl, location, value);
		...
		drawFrameBuffer: function(frameBuffer, renderFunc) {
